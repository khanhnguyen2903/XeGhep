    ...
    if data:
        for trip_id, info in data.items():
            ...
            trips.append({
                ...
                "driver_phone_assigned_trip": info.get('driver_phone_assigned_trip'),
                'status': info.get('status'),
            })

    return render(request, 'trips/list_trip.html', {'trips': trips, 'driver_name': driver_name, 'driver_coins': driver_coins})
...
def list_trip_receiving(request):
    ...
    if data:
        for trip_id, info in data.items():
            ...
            if (
                info.get('driver_phone_assigned_trip') == driver_phone and
                info.get('status') == "Đã nhận tài xế"
            ):
                dt = datetime.datetime.fromisoformat(info['pickup_time'])
                trips.append({
                    'id': trip_id,
...
def list_trip_completed(request):
    ...
    return render(request, "trips/list_trip.html", {
        "trips": trip_list,
        "driver_name": driver_name,
        "driver_coins": driver_coins,
    })
